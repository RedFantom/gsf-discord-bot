...
class DiscordBot(object):
    ...
    async def strategy(self, channel: Channel, user: DiscordUser, args: tuple):
        ...
        if command == "delete":
            ...
            title = "{}: {}".format(strategy.name, phase_name)
            embed = await self.build_embed(title, strategy.description, url)
            await self.bot.send_message(channel, embed=embed)

        else:
            await self.bot.send_message(channel, INVALID_ARGS)

    async def build_embed(self, title, description, image_url)->Embed:
        ...
        for elem in description.split("\n"):
            ...
            if len(elem) > 1024:
                elem = elem[:1020] + "..."
            self.logger.debug("Creating a new field: {}, {}".format(len(elem), elem[:10]))
            embed.add_field(name="**Description**", value=elem)
